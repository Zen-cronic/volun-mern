...
const EditEventForm = ({ event, eventId }) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      //count obj for multiple shifts in same date
      ...
      const shiftsWithListIds = event.shifts.map((shift) => {
        ...
        const correspondingEvent = eventDatesWithListIdsAndDates.find(
          (eventDateObj) => eventDateObj.date.includes(shiftStartDate)
            ...
            //try shift COunt prop  event 
        // if(eventDatesWithListIdsAndDates.find((eventDateObj) => eventDateObj.date === correspondingEvent.date && eventDateObj.listId === correspondingEvent.listId)){

        //   eventDatesWithListIdsAndDates = [...eventDatesWithListIdsAndDates, {
        //     listId: correspondingEvent.listId, 
        //     date: correspondingEvent.date,
  ...
  const renderEventDatesAndShifts = formData.shifts.map((shift) => {
    ...
    const correspondingEvent = formData.eventDates.find((eventDateObj) =>
      eventDateObj.listId === shift.parentListIdForShift
    );
