    ...
    if(isUserDataSuccess && isUpcomingShiftsSuccess && events){
        ...
        const upcomingShiftsArr = upcomingShifts.map((shiftEvent, idx )=> {
            ...
            const eventTableBodyContent = (
                <tr key={eventId}><EventExcerpt eventId={eventId} key={eventId}/></tr>)
        ...
        const volunteeredShiftsArrWithEvent = volunteer.volunteeredShifts.map((shiftObj)=> {

            const {shiftId, shiftDuration} = shiftObj

            const includedEvent = eventsArr?.find(event => {
    
                const foundEvent = event.shifts.find(shift=> (
    
                    shift._id.toString() === shiftId
                ))
            ...
            const includedShiftInfo = includedEvent.shifts.find(shift=> (
